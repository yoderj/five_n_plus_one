...
- this is actually not needed see paragraph below
You would also have to worry about the equivalence classes of the skipped powers, like when you skip 3^3 for 4^2, you’d have to verify that 4^5 is larger than 3^3, or also check 4^5. 

This is actually not required, since if you encounter the long string of zeros, it can only continue for one or 2 more digits past the passing point, since otherwise the growth would bee so high as to not be attainable by doing 5n+1. Since this limit is so tight as to be one or +1 or 2 in the resulting exponent, advancing one of the digits by a multiple like going from 4^2 to 4^5 to make sure everything going to 3^3 shrinks to do so is unnecessary.
