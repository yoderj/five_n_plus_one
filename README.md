...
3. the singular power of p1 (mod o': the order of p1 in modulus p2^n+1) to map to divisibility by p2^n+1 must have the same remainder (mod o: the order of p1 in the modulus p2^n) as the previous solution p1^s (assuming it existed). we know from 1 that out *n+1 mapping is a collisionless shuffling operation. there are p2 indexes mod p2^n+1 which map to 0 mod p2^n since before the mapping there were the indexes 0, p2^n, 2*p2^n...(p2-1)*p2^n which are 0 mod p2^n. from step 2 we know that o' = p2*o. therefore there are p2 solutions to the previous congruence in mod p2^n+1 namely p1^s *(1, p1^o, p1^2*o...p1^(o'-o)). since there are p2 solutions, p2 indexes under the map divisible by p2^n, and 0 was one of them, we know that one of those powers of p1 produces divisibility by p2^n+1. this reasoning defines an induction step on the conditions that the powers of p1 contain a solution for p2^n and the criteria for the lifting lemma are met.
